import type { MouseEvent } from 'react'
import { useWishlist } from '../../stores/WishlistContext'
import type { WishlistProductInput } from '../../stores/WishlistContext'
import styles from './WishlistButton.module.scss'

type WishlistButtonProps = {
  product: WishlistProductInput
  showLabel?: boolean
  className?: string
  onToggled?: (saved: boolean) => void
}

export function WishlistButton({
  product,
  showLabel = false,
  className = '',
  onToggled,
}: WishlistButtonProps) {
  const { isInWishlist, toggleWishlist } = useWishlist()
  const saved = isInWishlist(product.productId)

  const handleClick = (e: MouseEvent) => {
    e.preventDefault()
    e.stopPropagation()
    toggleWishlist(product)
    onToggled?.(!saved)
  }

  return (
    <button
      type="button"
      className={`${styles.btn} ${saved ? styles.saved : ''} ${className}`.trim()}
      aria-label={saved ? `Remove ${product.title} from wishlist` : `Save ${product.title} to wishlist`}
      aria-pressed={saved}
      onClick={handleClick}
    >
      <svg
        className={styles.icon}
        viewBox="0 0 24 24"
        aria-hidden
        fill={saved ? 'currentColor' : 'none'}
        stroke="currentColor"
        strokeWidth="2"
      >
        <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
      </svg>
      {showLabel && (
        <span className={styles.label}>{saved ? 'Saved' : 'Wishlist'}</span>
      )}
    </button>
  )
}
